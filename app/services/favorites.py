...
def read_all_favorites(user_id, db:Session) -> dict:
    favorite_table = db.query(Favorites.id, Favorites.added_at, Movies.title).filter(Favorites.user_id == user_id).join(Movies, Favorites.movie_id == Movies.id).all()
    return [{"id": r.id, "title": r.title, "added_at": r.added_at} for r in favorite_table]
